...
Sc_H2O=np.zeros(28) #to test whether nodes number is enough for establishing an accurate flux distribution
Sc_Mg=np.zeros(28)
...
trench=Nodes(N=200,alpha=90.0,AR=20,Sc_surface=Sc_surface)
[a,b]=trench.direct_flux_distribution(3.0098*0.1*1e4/np.sqrt(18),7.2362*0.1*1e4/np.sqrt(167.58))
plt.plot(a,'bo')
plt.plot(b,'ro')
plt.show()
...
'''
plt.plot(sticking_H2O,'ro',sticking_Mg,'bo')
plt.show()
'''
t0=time.time()
[F_H2O,F_Mg]=trench.stable_receiving_flux_vector(3.0098*0.1*1e4/np.sqrt(18),7.2362*0.1*1e4/np.sqrt(167.58),sticking_H2O,sticking_Mg,[a,b],2400)
t1=time.time()
print t1-t0

plt.plot(a,'r-',b,'b-',F_H2O,'ro',F_Mg,'bo')
plt.show()
